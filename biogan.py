...
class BIO_DCGAN(object):
    def __init__(self, dataset):
        ...
        # self.img_rows = math.ceil(np.sqrt(self.x_train.shape[1]))
    ...
    def plot_images(self, save2file=False, fake=True, samples=16, noise=None, step=0):
        filename = "{}_real.png".format(self.dataset)
        if fake:
            filename="{}_fake.png".format(self.dataset)
            if noise is None:
                noise = np.random.uniform(-1.0, 1.0, size=[samples, 100])
            else:
                filename = "{}_fake_{}.png".format(self.dataset, step)
